...
def _build_project_icon(icon_type, cx, cy, color):
    """Build a themed SVG icon for a project card.

    Args:
        icon_type: 'database', 'ai', 'terminal', or None
    ...
    if icon_type == "database":
        # Pipeline / database icon: stacked cylinders with flow lines
        return [
            f'    <ellipse cx="{cx}" cy="{cy - 4}" rx="6" ry="3" fill="none" stroke="{color}" stroke-width="1" opacity="0.7"/>',
            f'    <line x1="{cx - 6}" y1="{cy - 4}" x2="{cx - 6}" y2="{cy + 4}" stroke="{color}" stroke-width="0.8" opacity="0.5"/>',
            f'    <line x1="{cx + 6}" y1="{cy - 4}" x2="{cx + 6}" y2="{cy + 4}" stroke="{color}" stroke-width="0.8" opacity="0.5"/>',
            f'    <ellipse cx="{cx}" cy="{cy + 4}" rx="6" ry="3" fill="{color}" fill-opacity="0.15" stroke="{color}" stroke-width="1" opacity="0.7"/>',
        ...
        return [
            f'    <circle cx="{cx - 5}" cy="{cy - 4}" r="2.5" fill="{color}" opacity="0.7">',
            f'      <animate attributeName="opacity" values="0.4;1;0.4" dur="2s" begin="0s" repeatCount="indefinite"/>',
            f'    </circle>',
            f'    <circle cx="{cx + 5}" cy="{cy - 4}" r="2.5" fill="{color}" opacity="0.7">',
            f'      <animate attributeName="opacity" values="0.4;1;0.4" dur="2s" begin="0.6s" repeatCount="indefinite"/>',
            f'    </circle>',
            f'    <circle cx="{cx}" cy="{cy + 5}" r="2.5" fill="{color}" opacity="0.7">',
            f'      <animate attributeName="opacity" values="0.4;1;0.4" dur="2s" begin="1.2s" repeatCount="indefinite"/>',
            f'    </circle>',
            f'    <line x1="{cx - 4}" y1="{cy - 5}" x2="{cx + 4}" y2="{cy - 5}" stroke="{color}" stroke-width="0.6" opacity="0.35"/>',
            f'    <line x1="{cx - 3}" y1="{cy - 3}" x2="{cx}" y2="{cy + 4}" stroke="{color}" stroke-width="0.6" opacity="0.35"/>',
            f'    <line x1="{cx + 3}" y1="{cy - 3}" x2="{cx}" y2="{cy + 4}" stroke="{color}" stroke-width="0.6" opacity="0.35"/>',
        ]
    elif icon_type == "terminal":
        # Terminal icon: >_ prompt
        return [
            f'    <rect x="{cx - 8}" y="{cy - 6}" width="16" height="12" rx="2" fill="none" stroke="{color}" stroke-width="0.8" opacity="0.5"/>',
            f'    <circle cx="{cx - 4}" cy="{cy - 2.5}" r="1" fill="{color}" opacity="0.5"/>',
            f'    <text x="{cx + 1}" y="{cy + 2}" fill="{color}" font-size="7" font-family="monospace" font-weight="bold" opacity="0.9">&gt;_</text>',
        ]
    else:
        # Default: simple pulsing dot with ring
        return [
            f'    <circle cx="{cx}" cy="{cy}" r="8" fill="none" stroke="{color}" stroke-width="0.8" stroke-dasharray="4,3" opacity="0.5">',
            f'      <animateTransform attributeName="transform" type="rotate" from="0 {cx} {cy}" to="360 {cx} {cy}" dur="12s" repeatCount="indefinite"/>',
            f'    </circle>',
...
def _build_defs(n, card_width, gap, card_colors, theme):
    ...
    for i in range(n):
      ...
      <stop offset="0%" stop-color="{theme['grid']}" stop-opacity="0.6"/>
      <stop offset="100%" stop-color="{theme['lake_surface']}" stop-opacity="0.9"/>
    </linearGradient>''')
...
def _build_project_card(i, proj, layer, color, card_width, card_x, theme):
    ...
    category = proj.get("category", layer["name"])
    icon_type = proj.get("icon_type", None)
...
def render(projects: list, data_layers: list, theme: dict) -> str:
    """Render the projects constellation SVG.

    Args:
        projects: list of project dicts with repo, layer, description
        data_layers: list of layer configs for color mapping
  ...
  <text x="{WIDTH / 2}" y="{HEIGHT / 2}" fill="{theme['text_faint']}" font-size="12"
    ...
    for i in range(n):
        ...
        layer_idx = layer_idx if layer_idx < len(data_layers) else 0
        card_layers.append(layer_idx)
        card_colors.append(all_layer_colors[layer_idx])
    ...
    bg = (
        ...
        f'rx="12" ry="12" fill="{theme["depth"]}" '
