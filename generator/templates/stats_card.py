"""SVG template: Data Telemetry stats card (850x180)."""
...
def render(stats: dict, metrics: list, theme: dict) -> str:
    ...
    for i, key in enumerate(metrics):
      ...
      <g transform="translate(-8, -30) scale(1)">
        <svg viewBox="0 0 16 16" width="16" height="16" color="{icon_color}" class="metric-icon" style="animation-delay: {delay}">
          {icon_path}
        </svg>
      </g>
      ...
      <text x="0" y="20" text-anchor="middle" fill="{theme['text_faint']}" font-size="11" font-family="monospace" letter-spacing="1">{label}</text>
        ...
        if i < len(metrics) - 1:
            ...
            dividers.append(
                ...
                f'stroke="{theme["grid"]}" stroke-width="1" opacity="0.5"/>'
            )
  ...
  <!-- Section title -->
  <text x="30" y="38" fill="{theme['text_faint']}" font-size="11" font-family="monospace" letter-spacing="3">DATA TELEMETRY</text>
